...
#    desktop-file-edit --set-name="Software Store" "${entries_dir}"/io.github.kolunmi.Bazaar.desktop
    desktop-file-edit --set-name="Nemo File Manager" "${entries_dir}"/nemo.desktop
    desktop-file-edit --set-icon="fish" "${entries_dir}"/org.gnome.Ptyxis.desktop
    desktop-file-edit --set-icon="mintsources-maintenance" "${entries_dir}"/org.gnome.Settings.desktop
    desktop-file-edit --set-icon="np2" "${entries_dir}"/oneko.desktop
    desktop-file-edit --set-icon="/usr/share/icons/yazi.png" "${entries_dir}"/yazi.desktop || true

    # To use catcat update
#    desktop-file-edit --set-key="Exec" --set-value="/usr/bin/sudo /usr/bin/update all" \
#                      "${entries_dir}"/system-update.desktop
    desktop-file-edit --set-key="Exec" --set-value="/usr/bin/catcat-waydroid-launcher" \
                      "${entries_dir}"/Waydroid.desktop

    sed -i -E "s|^Exec=.*|Exec=nautilus --new-window Me/|" "${entries_dir}"/org.gnome.Nautilus.desktop
    desktop-file-edit --set-icon="user-home" --remove-key="DBusActivatable" \
                      "${entries_dir}"/org.gnome.Nautilus.desktop

    desktop-file-edit --remove-key="NoDisplay" "${entries_dir}"/gtk3-widget-factory.desktop
...
#        ['FontAwesome']="\
#$(get_ghpkg_url 'FortAwesome/Font-Awesome' 'desktop\.zip')"
...
install_icon_themes() {
    ...
    local JQ_FILTER icons_url icons_archive papirus_subd papirus_dir
    ...
    if [[ -d "${ICONS_DIR}/Papirus" ]]; then
        ...
        icons_url="$(get_ghpkg_url 'PapirusDevelopmentTeam/papirus-icon-theme')"
        icons_archive="/tmp/icons/$(basename "${icons_url}").tar"

        mkdir -vp "$(dirname "${icons_archive}")"
        curl_get "${icons_archive}" "${icons_url}"
...
install_gtk_themes() {
    ...
    if [[ -d "${THEMES_DIR}/Lavanda-Dark" ]]; then
        ...
        JQ_FILTER='.tarball_url'
        theme_url="$(get_ghpkg_url 'vinceliuice/Lavanda-gtk-theme')"
...
apply_default_configs() {
    ...
    sed -i 's|^[#[:space:]]*Inherits=.*|Inherits=Catppuccin-Papirus-Orange|' \
