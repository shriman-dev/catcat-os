...
check_file_presence /usr/lib/dracut/dracut.conf.d/99-omit-firewire-thunderbolt.conf

log "INFO" "Checking modprobe blacklist"
check_file_presence /usr/lib/modprobe.d/secureblue.conf
check_file_presence /usr/lib/modprobe.d/secureblue-framebuffer.conf

log "INFO" "Checking sysctl configuration"
check_file_presence /usr/lib/sysctl.d/99-catcat-sysctl.conf
...
sed -i -Ee 's|^[#[:space:]]*Storage=.*|Storage=none|' \
        -e 's|^[#[:space:]]*ProcessSizeMax=.*|ProcessSizeMax=0|' \
        -e 's|^[#[:space:]]*ExternalSizeMax=.*|ExternalSizeMax=0|' \
...
check_file_presence "${networkmanager_confd}/mac-randomization.conf" \
                    "${networkmanager_confd}/privacy_ext_ipv6.conf"

# Harden SSH
log "INFO" "Checking harden SSH configuration"
check_file_presence "/etc/ssh/sshd_config.d/catcat-ssh.conf"
...
check_file_presence "/etc/${PROJECT_NAME}/localdns.d/config.json"
...
sed -i -Ee 's|^UMASK([[:space:]]+).*|UMASK\1027|' \
        -e 's|^YESCRYPT_COST_FACTOR([[:space:]]+).*|YESCRYPT_COST_FACTOR\18|' "${login_defs}"
...
log "INFO" "Improving password quality checks"
...
check_file_presence "${mute_mic_file}"
