#!/usr/bin/env bash
source "${BUILD_SETUP_DIR}/lib/bash-lib.sh"
...
kernel_add() {
    { brief_trace; } 2>/dev/null
    "${BUILD_SETUP_DIR}/shared/pkgs-kernel.sh"
    { brief_trace; } 2>/dev/null
}

pkgs_hwaccel() {
    { brief_trace; } 2>/dev/null
    "${BUILD_SETUP_DIR}/shared/pkgs-hwaccel.sh"
    { brief_trace; } 2>/dev/null
}
