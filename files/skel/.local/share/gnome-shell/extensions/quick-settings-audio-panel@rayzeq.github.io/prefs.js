...
export default class QSAPPreferences extends ExtensionPreferences {
    fillPreferencesWindow(window) {
        ...
        const row = create_switch(settings, 'create-mixer-sliders', {
        ...
        main_group.add(create_switch(settings, 'separate-indicator', {
        ...
        mixer_filter_group.add(create_dropdown(settings, 'filter-mode', {
        ...
        sink_mixer_filter_group.add(create_dropdown(settings, 'sink-filter-mode', {
            title: _("Filtering mode"),
            subtitle: _("On blacklist mode, matching elements are removed from the list. On whitelist mode, only matching elements will be shown"),
            fields: [
                ['blacklist', _("Blacklist")],
        ...
        libpanel_group.add(create_switch(libpanel_settings, 'single-column', {
        ...
        libpanel_group.add(create_dropdown(libpanel_settings, 'alignment', {
        ...
        libpanel_group.add(create_switch_spin(libpanel_settings, 'padding-enabled', 'padding', {
            title: _("Padding"),
            subtitle: _("Use this to override the default padding of the panels")
        }, 0, 100));
        libpanel_group.add(create_switch_spin(libpanel_settings, 'row-spacing-enabled', 'row-spacing', {
            title: _("Row spacing"),
            subtitle: _("Use this to override the default row spacing of the panels")
        }, 0, 100));
        libpanel_group.add(create_switch_spin(libpanel_settings, 'column-spacing-enabled', 'column-spacing', {
...
function create_switch_spin(settings, switch_id, spin_id, options, lower = 0, higher = 100) {
    ...
    settings.bind(switch_id, switch_, 'active', Gio.SettingsBindFlags.DEFAULT);
    row.add_prefix(switch_);
    row.activatable_widget = switch_;
    settings.bind(spin_id, row, 'value', Gio.SettingsBindFlags.DEFAULT);
    return row;
}
function create_file_chooser_row(settings, id, options) {
    const row = new Adw.EntryRow({ ...options, show_apply_button: false });
    settings.bind(id, row, 'text', Gio.SettingsBindFlags.DEFAULT);
