...
export function add_named_connections(patcher, object) {
    ...
    function set_signal(object, source, signal, callback, id) {
        ...
        if (!signal_map.has(signal))
            signal_map.set(signal, new Map());
...
export function get_settings(path) {
    ...
    const source = Gio.SettingsSchemaSource.new_from_directory(parent_path, Gio.SettingsSchemaSource.get_default(), false);
