...
if systemctl is-active gdm && [[ -n "${DISPLAY}" && "${dash_to_panel_positions}" =~ '"":' ]]; then
    primary_monitor="$(gdbus call --session --dest org.gnome.Mutter.DisplayConfig \
                       --object-path /org/gnome/Mutter/DisplayConfig \
                       --method org.gnome.Mutter.DisplayConfig.GetCurrentState | \
                       grep -o '\[((.*), \[(' | \
                       tr -cd '[:alnum:],' | cut -d ',' -f2,4 | tr ',' '-')"
