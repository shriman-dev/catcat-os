...
if [[ -n "${DISPLAY}" ]]; then
if ! grep -q "${primary_monitor}" <<< "${dash_to_panel_positions}"; then
...
dconf write /org/gnome/shell/extensions/dash-to-panel/panel-element-positions $(grep -E "^.*panel-element-positions=.*$" "${usr_dconf_ext}" | sed "s|panel-element-positions='{\".*\":\[{\"|panel-element-positions='{\"${primary_monitor}\":\[{\"|" | sed "s|panel-element-positions=||")

dconf write /org/gnome/shell/extensions/dash-to-panel/panel-positions $(grep -E "^.*panel-positions=.*$" "${usr_dconf_ext}" | sed "s|panel-positions='{\".*\":|panel-positions='{\"${primary_monitor}\":|" | sed "s|panel-positions=||")
