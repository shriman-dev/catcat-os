...
unzip /tmp/yazi.zip
cp -Pvf yazi-x86_64-unknown-linux-gnu/yazi /usr/bin/
...
cp -Pvf /tmp/ascii-image-converter/ascii-image-converter /usr/bin/
chmod +x /usr/bin/ascii-image-converter

# pipes.sh
git clone https://github.com/pipeseroni/pipes.sh.git /tmp/pipes.sh
cp -Pvf /tmp/pipes.sh/pipes.sh /usr/bin/

# pokemonsay-newgenerations
git clone https://github.com/HRKings/pokemonsay-newgenerations.git /tmp/pokemonsay-newgenerations

cp -Prf /tmp/pokemonsay-newgenerations/pokemons /usr/bin/
cp -Pvf /tmp/pokemonsay-newgenerations/pokemonsay.sh /usr/bin/
cp -Pvf /tmp/pokemonsay-newgenerations/pokemonthink.sh /usr/bin/
